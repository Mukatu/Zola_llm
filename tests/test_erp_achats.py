...
class _CapturingClient(LLMClient):
    ...
    async def generate(self, messages, *, model, options=None):  # type: ignore[no-untyped-def]
        ...
        return GenerationResult(content="Contrat OHADA rédigé.", model="fake", provider=self.provider)
...
def _offre(idx: str, fourn: str, ttc: str, delai: int) -> OffreFournisseur:
    return OffreFournisseur(
        id_externe=idx, fournisseur=fourn, objet="Consommables",
        montant_ht_xaf=Decimal(ttc), montant_ttc_xaf=Decimal(ttc), delai_livraison_jours=delai,
    )
...
def test_verifier_conformite() -> None:
    ...
    s2 = Supplier(id_externe="F2", nom="Beta", documents_conformite=["rccm", "niu", "attestation_fiscale"])
    assert verifier_conformite(s2) == []


def test_score_fournisseur() -> None:
    bon = Supplier(id_externe="F1", nom="Alpha", note_qualite=Decimal("5"),
                   delai_moyen_jours=0, documents_conformite=list(("rccm", "niu", "attestation_fiscale")))
    mauvais = Supplier(id_externe="F2", nom="Beta", note_qualite=Decimal("1"),
                       delai_moyen_jours=30, documents_conformite=[])
...
async def test_agent_redige_contrat_sans_inventer_montant() -> None:
    ...
    out = await agent.rediger_contrat(fournisseur="Beta", objet="Fourniture de gants", montant_xaf="800000")
...
async def test_agent_synthese_comparatif() -> None:
    agent = AchatsAgent(client=_CapturingClient(), settings=Settings())
    classement = comparer_offres([_offre("O1", "Beta", "800000", 5), _offre("O2", "Alpha", "1000000", 10)])
