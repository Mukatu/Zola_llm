...
def _tx(idx: str, montant: str, sens: str, d: date = date(2026, 1, 10), libelle: str = "Paiement") -> BankTransaction:
    return BankTransaction(
        id_externe=idx, date_operation=d, libelle=libelle,
        montant_xaf=Decimal(montant), sens=sens, canal="bank",
    )


def _inv(idx: str, ttc: str, *, echeance: date | None, payee: bool = False) -> Invoice:
    return Invoice(
        id_externe=idx, numero=f"F-{idx}", tiers="ACME", date_emission=date(2025, 12, 1),
        date_echeance=echeance, montant_ht_xaf=Decimal(ttc), montant_ttc_xaf=Decimal(ttc), payee=payee,
    )
...
class _FakeFinanceConnector(FinanceConnector, InvoiceConnector):
    ...
    async def list_bank_transactions(self, **f: Any) -> list[BankTransaction]:
        return [_tx("T1", "500000", "credit"), _tx("T2", "1500000", "debit"),
                _tx("T3", "200000", "debit", libelle="Loyer"), _tx("T4", "200000", "debit", libelle="Loyer")]
...
def test_detect_duplicates(agent: FinanceAgent) -> None:
    txs = [_tx("A", "200000", "debit", libelle="Loyer"), _tx("B", "200000", "debit", libelle="Loyer")]
...
def test_detect_overdue_invoices(agent: FinanceAgent) -> None:
    invs = [_inv("I1", "300000", echeance=date(2025, 12, 1)),
...
async def test_analyze_from_connector(settings: Settings) -> None:
    agent = FinanceAgent(client=_CapturingClient("x"), settings=settings, connector=_FakeFinanceConnector())
    res = await agent.analyze_from_connector(large_threshold_xaf=Decimal("1000000"), as_of=date(2026, 1, 15))
